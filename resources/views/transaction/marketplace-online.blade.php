{{--
  ...
  Versi ini menambahkan tombol "Detail" yang memunculkan modal untuk melihat
  daftar barang dalam pesanan hutang (online), serta memperbarui status
  transaksi ketika proses pembayaran selesai.
--}}
...
<x-layout>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-striped" id="online-orders-table">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Faktur</th>
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center text-muted">Belum ada pesanan online yang menunggu.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div class="modal fade" id="transaction_detail_modal" tabindex="-1" aria-labelledby="detailLabel" aria-hidden="true">
    ...
    @push('scripts')
        <script>
            ...
            function showDetail(btn) {
                ...
                $.get(`/transaction/online-orders/${id}/items`, function(html) {
            ...
            // Hitung kembalian saat input uang tunai berubah
            ...
            // Kirim permintaan pemrosesan pembayaran
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.status === 'success') {
                            ...
                            // Hapus baris dari DataTable setelah sukses
                            const row = $('#online-orders-table button[data-id="' + id + '"]').closest('tr');
                            if ($.fn.DataTable && $('#online-orders-table').DataTable().row) {
                                $('#online-orders-table').DataTable().row(row).remove().draw();
                            } else {
                                row.remove();
                            }
                        } else {
